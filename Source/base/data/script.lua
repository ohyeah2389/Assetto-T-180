...
DEBUG = ac.getCarName(car.index) == "T-180 Chassis DEV"
...
local steeringRangeSetup = ac.getScriptSetupValue("CUSTOM_SCRIPT_ITEM_20") or {}
...
function script.update(dt)
    ...
    if config.turbojet.present then
        if config.turbojet.type == "single" and turbojetCenter then
            ...
            Data.controllerInputs[8] = helpers.mapRange(turbojetCenter.throttle, config.turbojet.minThrottle, 1, 0, 1, true)                     -- throttle
            Data.controllerInputs[9] = helpers.mapRange(turbojetCenter.thrust, 1000, 8000, 0, 1, true)                                           -- thrust
            Data.controllerInputs[10] = turbojetCenter.shaft.angularSpeed * 60 / (2 * math.pi)                                                   -- rpm
            Data.controllerInputs[11] = turbojetCenter.fuelPumpEnabled and 1 or 0                                                                -- fuel pump enabled
            Data.controllerInputs[12] = helpers.mapRange(turbojetCenter.thrustAfterburner, 0, 2000, 0, 1, true)                                  -- burner throttle
            Data.controllerInputs[18] = helpers.mapRange(math.max(turbojetCenter.heatFrame, turbojetCenter.heatCore / 2), 700, 1000, 0, 1, true) -- damage

            -- Set heat controller channels
            Data.controllerInputs[21] = turbojetCenter.heatCore
            Data.controllerInputs[22] = turbojetCenter.heatFrame
        elseif config.turbojet.type == "dual" and turbojetLeft and turbojetRight then
            -- Set throttles
            turbojetLeft.targetThrottle = math.clamp(Data.gas + helpers.mapRange(Data.steer, 0, 0.4, 0, 0.5, true) - helpers.mapRange(Data.steer, -0.4, 0, 0.25, 0, true), config.turbojet.minThrottle, 1)
            turbojetRight.targetThrottle = math.clamp(Data.gas + helpers.mapRange(Data.steer, -0.4, 0, 0.5, 0, true) - helpers.mapRange(Data.steer, 0, 0.4, 0, 0.25, true), config.turbojet.minThrottle, 1)
            ...
            -- Set controller channels
            Data.controllerInputs[8] = helpers.mapRange(turbojetLeft.throttle, config.turbojet.minThrottle, 1, 0, 1, true)   -- throttle
            Data.controllerInputs[9] = helpers.mapRange(turbojetLeft.thrust, 1000, 8000, 0, 1, true)                         -- thrust
            Data.controllerInputs[10] = turbojetLeft.shaft.angularSpeed * 60 / (2 * math.pi)                                 -- rpm
            Data.controllerInputs[11] = turbojetLeft.fuelPumpEnabled and 1 or 0                                              -- fuel pump enabled
            Data.controllerInputs[12] = helpers.mapRange(turbojetLeft.thrustAfterburner, 0, 2000, 0, 1, true)                -- burner throttle
            Data.controllerInputs[13] = helpers.mapRange(turbojetRight.throttle, config.turbojet.minThrottle, 1, 0, 1, true) -- secondary throttle
            Data.controllerInputs[14] = helpers.mapRange(turbojetRight.thrust, 1000, 8000, 0, 1, true)                       -- secondary thrust
            Data.controllerInputs[15] = turbojetRight.shaft.angularSpeed * 60 / (2 * math.pi)                                -- secondary rpm
            Data.controllerInputs[16] = turbojetRight.fuelPumpEnabled and 1 or 0                                             -- secondary fuel pump enabled
            Data.controllerInputs[17] = helpers.mapRange(turbojetRight.thrustAfterburner, 0, 2000, 0, 1, true)               -- secondary burner throttle

            -- Set controller channels for damage
            Data.controllerInputs[18] = helpers.mapRange(math.max(turbojetLeft.heatFrame, turbojetLeft.heatCore / 2), 700, 1000, 0, 1, true)   -- damage
            Data.controllerInputs[19] = helpers.mapRange(math.max(turbojetRight.heatFrame, turbojetRight.heatCore / 2), 700, 1000, 0, 1, true) -- secondary damage

            -- Set heat controller channels
            Data.controllerInputs[21] = math.max(turbojetLeft.heatCore, turbojetRight.heatCore)
            Data.controllerInputs[22] = math.max(turbojetLeft.heatFrame, turbojetRight.heatFrame)
    ...
    if config.turboshaft.present then
        if config.turboshaft.type == "dual" and turboshaftFront and turboshaftRear and drivetrainFront and drivetrainRear then
            -- Update drivetrains first to calculate slip
            local slipFront = (Data.wheels[ac.Wheel.FrontLeft].ndSlip + Data.wheels[ac.Wheel.FrontRight].ndSlip) / 2
            local slipRear = (Data.wheels[ac.Wheel.RearLeft].ndSlip + Data.wheels[ac.Wheel.RearRight].ndSlip) / 2
            ...
            turboshaftFront.throttle = math.clamp(Data.gas + (ac.isControllerGearUpPressed() and 0.75 or 0) - overLimitFront, 0, 1)
            turboshaftRear.throttle = math.clamp(Data.gas + (ac.isControllerGearDownPressed() and 0.75 or 0) - overLimitRear, 0, 1)
            ...
            Data.controllerInputs[13] = helpers.mapRange(turboshaftFront.throttle, config.turbojet.minThrottle, 1, 0, 1, true)
            Data.controllerInputs[14] = turboshaftFront.fuelSystem.actualFuelFlow / turboshaftFront.fadec.maxFuelFlow
            Data.controllerInputs[15] = ((turboshaftFront.gasTurbine.angularSpeed * 60 / (2 * math.pi)) or 0) * (10000 / 45000)
            Data.controllerInputs[16] = turboshaftFront.fuelSystem.pumpEnabled and 1 or 0
            Data.controllerInputs[17] = turboshaftFront.afterburner.throttleAfterburner
            Data.controllerInputs[18] = frontDamage

            -- Update controller channels for rear turbine
            Data.controllerInputs[8] = helpers.mapRange(turboshaftRear.throttle, config.turbojet.minThrottle, 1, 0, 1, true)
            Data.controllerInputs[9] = turboshaftRear.fuelSystem.actualFuelFlow / turboshaftRear.fadec.maxFuelFlow
            Data.controllerInputs[10] = ((turboshaftRear.gasTurbine.angularSpeed * 60 / (2 * math.pi)) or 0) * (10000 / 45000)
            Data.controllerInputs[11] = turboshaftRear.fuelSystem.pumpEnabled and 1 or 0
            Data.controllerInputs[12] = turboshaftRear.afterburner.throttleAfterburner
            Data.controllerInputs[19] = rearDamage

            -- Set heat controller channels
            Data.controllerInputs[21] = turboshaftRear.sensors.turbineInletTemp
            Data.controllerInputs[22] = turboshaftRear.gasTurbine.angularSpeed
            ...
            local frontFeedback = drivetrainFront:update(turboshaftFront.outputRPM, turboshaftFront.outputTorque, math.clamp(turboshaftFront.throttle - Data.brake, 0, 1), dt)
            local rearFeedback = drivetrainRear:update(turboshaftRear.outputRPM, turboshaftRear.outputTorque, math.clamp(turboshaftRear.throttle - Data.brake, 0, 1), dt)
            turboshaftFront.feedbackTorque = frontFeedback
            turboshaftRear.feedbackTorque = rearFeedback
        elseif turboshaftFront and drivetrainRear then
            turboshaftFront.throttle = Data.gas
            turboshaftFront:update(dt)

            -- Update controller channels
            Data.controllerInputs[8] = helpers.mapRange(turboshaftFront.throttle, config.turbojet.minThrottle, 1, 0, 1, true)
            Data.controllerInputs[9] = helpers.mapRange(turboshaftFront.outputTorque, 0, 2000, 0, 1, true)
            Data.controllerInputs[10] = turboshaftFront.outputRPM / 2.5
            Data.controllerInputs[11] = turboshaftFront.afterburner.throttleAfterburner
            Data.controllerInputs[12] = turboshaftFront.throttleAfterburner
            ...
            -- Update drivetrains with turbine output
            local drivetrainFeedback = drivetrainRear:update(turboshaftFront.outputRPM, turboshaftFront.outputTorque, Data.clutch, dt)
            turboshaftFront.feedbackTorque = drivetrainFeedback
        end
    end

    -- If we're drifting backwards, don't allow clutch input
    if state.control.driftInversion then Data.clutch = 0 end

    -- If the rear-mechanical-steer linkage ratio setup option exists, transmit it over its controller channel
    if linkageRatioSetup then Data.controllerInputs[20] = linkageRatioSetup.value end
    ...
    local rollForce = math.clamp(Data.steer / (90 * steeringRangeSetup.value / 180), -1, 1) * 100 * (1 - suctionMult)
