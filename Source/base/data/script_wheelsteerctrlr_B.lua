...
function WheelSteerCtrlr:update(dt)
    ...
    local pidSteerRL = self.RL_slipTargetPID:update(slipOffsetRL, -math.clamp(slipAngleRL, -0.5, 0.5), dt) * helpers.mapRange(car.speedKmh, 10, 60, 0.2, 1, true)
    local pidSteerRR = self.RR_slipTargetPID:update(slipOffsetRR, -math.clamp(slipAngleRR, -0.5, 0.5), dt) * helpers.mapRange(car.speedKmh, 10, 60, 0.2, 1, true)
    ...
    ac.debug("steerctrl.rotations", self.rotations)
